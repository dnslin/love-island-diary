...
src/
  ...
  lib/              # Server Actions, Prisma client, schemas, test helpers
  hooks/            # Custom React hooks with co-located tests
  types/            # Empty — types live in lib/schemas.ts
...
- **No middleware.ts**: Auth/initialization guard is in `(protected)/layout.tsx` via `redirect('/settings')`.
